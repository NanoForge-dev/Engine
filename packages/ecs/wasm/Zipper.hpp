...
namespace nfo {
    class Zipper {
      public:
        explicit Zipper(const std::map<std::string, SparseArray<emscripten::val> *> &arrays) : _arrays(arrays), _max(0), _idx(0)
        {
      ...
      private:
        ...
        [[nodiscard]] bool all_set() const
        {
            ...
            return std::ranges::all_of(_arrays | std::views::values, [this](SparseArray<emscripten::val> *const &arr) {
                return (*arr)[_idx].has_value();
            });
        }
